...
class EmailVerificationGuard {
  ...
  async init() {
    ...
    if (PUBLIC_PAGES.includes(currentPage)) {
      console.log('🚫 Skipping guard for public page:', currentPage);
      return;
    }

    console.log('🛡️ Email verification guard initialized for:', currentPage);
    
    // Show loading state while checking authentication
    this.showLoadingState();
    
    // Small delay to allow any login redirects to settle
    await new Promise(resolve => setTimeout(resolve, 500));
    
    // Wait for Firebase to be available from app.js
    await this.waitForGlobalFirebase();
    this.checkEmailVerification();
  }

  async waitForGlobalFirebase() {
    return new Promise((resolve, reject) => {
      let attempts = 0;
      const maxAttempts = 30; // Reduced to 3 seconds max wait
      
      const checkForAuth = () => {
        attempts++;
        console.log(`Auth check attempt ${attempts}/${maxAttempts}`, {
          windowAuth: !!window.auth,
          windowAppAuth: !!window.appAuth,
          windowFirebase: !!window.firebase
        });
        
        // Check if global auth is available
        if (window.appAuth) {
          console.log('✅ Found window.appAuth:', window.appAuth);
          resolve();
          return;
        }
        
        if (window.auth) {
          console.log('✅ Found window.auth:', window.auth);
          resolve();
          return;
        }
        
        if (window.firebase && window.firebase.auth) {
          console.log('✅ Found window.firebase.auth');
          resolve();
          return;
        }
        
        if (attempts >= maxAttempts) {
          console.error('❌ Firebase auth not found after waiting, available globals:', Object.keys(window));
          this.hideLoadingState();
          this.redirectToLogin('System initialization error. Please refresh the page.');
          reject(new Error('Firebase auth not found'));
          return;
        }
  ...
  async checkEmailVerification() {
    try {
      ...
      console.log('✅ Firebase auth module imported successfully');
      
      // Get auth instance from global scope or imports
      let authInstance = window.appAuth || window.auth;
      console.log('🔑 Auth instance found:', !!authInstance);
      
      if (!authInstance) {
        // Try to get from app.js if it was imported
        try {
          const appModule = await import('./app.js');
          authInstance = appModule.auth;
          console.log('📦 Got auth from app.js module:', !!authInstance);
        } catch (e) {
          console.error('❌ Could not import app.js:', e);
        }
      }
      ...
      // Check if user just logged in (give extra time for auth state to settle)
      const urlParams = new URLSearchParams(window.location.search);
      const justLoggedIn = urlParams.has('login_success') || sessionStorage.getItem('justLoggedIn');
      
      if (justLoggedIn) {
        console.log('🔑 User just logged in, giving extra time for auth state...');
        sessionStorage.removeItem('justLoggedIn');
        // Give more time for auth state to settle after login
      ...
      onAuthStateChanged(authInstance, (user) => {
        ...
        if (!user) {
          // Don't redirect immediately if user just logged in - Firebase might still be loading
          if (justLoggedIn) {
            console.log('⏳ User just logged in but no auth state yet, waiting...');
            setTimeout(() => {
  ...
  redirectToLogin(message = 'Authentication required') {
    ...
    if (sessionStorage.getItem('redirecting_to_login')) {
      console.log('🔄 Preventing redirect loop to login');
      return;
    }
  ...
  async setupVerificationPageEvents(user) {
    ...
    if (!authInstance) {
      try {
        ...
        console.error('Could not get auth instance for events:', e);
      }
    }
